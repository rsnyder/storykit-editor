...
SITE = REPO / "_site"
# Exercise the actual Worker policy inherited by its srcdoc preview.
...
def _route_site(route):
    ...
    if f.exists() and f.is_file():
        route.fulfill(status=200, body=f.read_bytes(),
                      ...
                      headers={"Content-Security-Policy": EDITOR_CSP} if rel == "editor/index.html" else {})
    else:
...
def _dialog_state(page):
    return page.frame_locator("#preview-mount iframe.pv-frame").locator("body").evaluate("""body => {
        const sl = body.ownerDocument.getElementById('storykitDialog');
        return { present: !!sl,
...
EXPAND_CASES = {
    ...
    "youtube": (
        ...
        "iframe.embed-youtube", ".click-overlay"),
    "image-compare": (
...
def test_action_link_zoomto_reaches_the_component(editor_prod):
    ...
    got = None
    for _ in range(20):
        got = v.locator("body").evaluate("""body => {
            return body.ownerDocument.defaultView.__actions;
        }""")
        if got:
            break
        page.wait_for_timeout(250)
...
def test_expand_works_when_editor_is_cross_origin_with_components(playwright, built_site):
    ...
    try:
        ...
        comp = page.frame_locator("#preview-mount iframe.pv-frame").locator("body").evaluate("""body => {
...
def test_footnote_click_stays_in_document(editor_prod):
    ...
    state = frame.locator("body").evaluate(
        """body => {
            const d = body.ownerDocument;
            return {
                stillSrcdoc: d.location.href.startsWith('about:srcdoc'),
                hasFootnotes: !!d.querySelector('.footnotes, #fn1, [id^=fn]'),
                scrolled: d.defaultView.scrollY > 100,
...
def test_preview_cannot_read_editor_storage(editor_prod):
    page, load = editor_prod
    page.evaluate("() => localStorage.setItem('isolation-probe', 'editor-only')")
    load(FRONT + 'An isolated draft.\n')
    frame = page.locator('#preview-mount iframe.pv-frame').element_handle().content_frame()
    state = frame.evaluate("""() => {
        let parentBlocked = false, databaseBlocked = false;
        try { parent.localStorage.getItem('isolation-probe'); } catch { parentBlocked = true; }
        try { indexedDB.open('storykit-editor'); } catch { databaseBlocked = true; }
        localStorage.setItem('isolation-probe', 'preview-only');
        return {origin: window.origin, parentBlocked, databaseBlocked,
                local: localStorage.getItem('isolation-probe')};
    }""")
    assert state == {'origin': 'null', 'parentBlocked': True,
                     'databaseBlocked': True, 'local': 'preview-only'}
    assert page.evaluate("() => localStorage.getItem('isolation-probe')") == 'editor-only'
