...
def _set_token(page, token):
    """Seed a legacy PAT to prove the new session client never transmits it."""
    page.evaluate("""async token => {
      localStorage.setItem('jekyllPreviewPAT', token);
      await (await import('/editor/auth.js')).refresh();
    }""", token)
...
def _bind(page, owner, repo, branch, path):
    ...
    page.locator("#sync-repositories option[value=\"1\"]").wait_for(state="attached")
    page.locator("#sync-repositories").select_option("1")
    page.wait_for_function("() => document.getElementById('sync-branch').value !== ''")
...
def test_token_never_appears_in_any_request_url(browser, site_dir):
    with rr.serve_site(site_dir) as base_url:
        ...
        try:
            ...
            assert not any(r["authorization"] for r in mock.request_log), "browser must never transmit GitHub credentials"
            assert any("/api/github/save" in r["url"] for r in mock.request_log), "writes use the session service"
        finally:
            context.close()
