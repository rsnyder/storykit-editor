...
def test_workflow4_publish_sync_panel_bind_and_commit_keyboard_only(browser, site_dir):
    with rr.serve_site(site_dir) as base_url:
        ...
        try:
            ...
            # The session is supplied by the mock; keyboard users choose a repo.
            branch = "main"
            path = "_posts/2026-07-06-keyboard-workflow.md"
            page.locator('#sync-repositories option[value="1"]').wait_for(state="attached")
            page.locator("#sync-repositories").focus()
            page.keyboard.press("Home")
            page.keyboard.press("ArrowDown")
            page.keyboard.press("Tab")
            page.wait_for_function("() => document.getElementById('sync-repositories').value === '1' && document.getElementById('sync-branch').value === 'main'")
            page.locator("#sync-branch").focus()
            page.locator("#sync-branch").fill(branch)
