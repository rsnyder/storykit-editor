...
def test_editor_preview_matches_golden(mv_capture):
    ...
    assert captured == golden, (
        f"editor srcdoc diverges from the editor golden "
        f"({len(captured)} vs {len(golden)} bytes) — the editor changed its "
        f"render pipeline; run tools/render_regression.py --check "
