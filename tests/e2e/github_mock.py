...
class GitHubMock:
    ...
    def __init__(self, page, owner: str, repo: str, default_branch: str = "main"):
        ...
        page.route("**/api/**", self._service)
        page.route(prefix, self._handle)
        page.route(f"{prefix}/**", self._handle)

    def _service(self, route):
        req = route.request
        url = urllib.parse.urlparse(req.url)
        data = json.loads(req.post_data or "{}") if req.method == "POST" else dict(urllib.parse.parse_qsl(url.query))
        self.request_log.append({"method": req.method, "url": req.url, "authorization": req.headers.get("authorization")})
        ...
        if endpoint == "/api/session":
            reply({"configured": True, "user": {"id": 1, "login": "test-author"} if self.signed_in else None, "csrf": "test-csrf", "scope": "repo"}); return
        if endpoint == "/api/auth/logout":
            self.signed_in = False; reply({"ok": True}); return
        if not self.signed_in:
            reply({"error": {"message": "Sign in again", "code": "signed_out"}}, 401); return
        if req.method == "POST" and req.headers.get("x-csrf-token") != "test-csrf":
            reply({"error": {"message": "Missing CSRF"}}, 403); return
        if endpoint == "/api/github/repos":
            reply({"items": [{"id": 1, "owner": self.owner, "name": self.repo, "fullName": f"{self.owner}/{self.repo}", "defaultBranch": self.default_branch, "private": False}], "nextPage": None}); return
        if data.get("owner") != self.owner or data.get("repo") != self.repo:
            reply({"error": {"message": "Not found"}}, 404); return
        branch = data.get("branch", self.default_branch)
        if endpoint == "/api/github/repo":
            reply({"default_branch": self.default_branch, "permissions": {"push": True}})
        elif endpoint == "/api/github/branches":
            reply({"items": [{"name": b} for b in self.branches], "nextPage": None})
        elif endpoint == "/api/github/branch-head":
            reply({"sha": self._branch_head_sha.get(branch)})
        elif endpoint == "/api/github/create-branch":
            if branch in self.branches:
                reply({"error": {"message": "Branch already exists"}}, 422); return
            source = next((b for b, sha in self._branch_head_sha.items() if sha == data.get("fromSha")), self.default_branch)
            self.branches[branch] = copy.deepcopy(self.branches[source])
            self._branch_head_sha[branch] = data["fromSha"]
            reply({"sha": data["fromSha"], "ref": f"refs/heads/{branch}"})
        elif endpoint in ("/api/github/read", "/api/github/file"):
            reply({"file": self.get_remote(branch, data["path"])})
        elif endpoint == "/api/github/save":
            current = self.get_remote(branch, data["path"])
            if (current or {}).get("sha") != data.get("expectedSha"):
                reply({"error": {"message": "File changed", "code": "file_changed"}}, 409); return
            sha = self.set_remote(branch, data["path"], data["content"])
            reply({"sha": sha})
        else:
            reply({"error": {"message": "Unknown service endpoint"}}, 404)
