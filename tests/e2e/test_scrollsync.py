...
def _boot_split(page, base_url):
    ...
    page.frame_locator('#preview-mount iframe.pv-frame').locator('.post-content h2').nth(9).wait_for(timeout=60_000)
    page.wait_for_timeout(1500)


def _preview_frame(page):
    return page.locator('#preview-mount iframe.pv-frame').element_handle().content_frame()


def _preview_scroll_y(page):
    return _preview_frame(page).evaluate("() => scrollY")
...
def test_two_way_scroll_sync_in_split_view(browser, site_dir):
    with rr.serve_site(site_dir) as base_url:
        ...
        try:
            ...
            _preview_frame(page).wait_for_function("() => scrollY > 300", timeout=10_000)
            ...
            _preview_frame(page).evaluate("() => scrollTo({ top: 50 })")
