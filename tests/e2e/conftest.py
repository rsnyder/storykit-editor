...
class _QuietHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        component = re.fullmatch(r'/assets/components/(image|map)(\.html)?', path)
        if component and component[2]:
            self.send_response(307)
            self.send_header('Location', '/assets/components/' + component[1] + (sep + query if sep else ''))
            self.end_headers()
            return
        if component:
            self.path = '/assets/components/' + component[1] + '.html' + (sep + query if sep else '')
        super().do_GET()

    def end_headers(self):
        # Match the deployed public module headers for opaque preview frames.
        if self.path.startswith('/assets/js/'):
            self.send_header('Access-Control-Allow-Origin', '*')
        if re.match(r'/assets/components/(image|map)(?:\.html)?(?:\?|$)', self.path):
            worker = (REPO / 'prototypes/github-auth/src/worker.js').read_text()
            policy = re.search(r'"(sandbox allow-scripts[^"\n]+)"', worker).group(1)
            policy = policy.replace("script-src 'self' ", f"script-src 'self' http://{self.headers['Host']}/assets/js/ ")
