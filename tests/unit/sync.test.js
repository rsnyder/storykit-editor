...
describe('sync: bindDocument (FR-GH.2)', () => {
  ...
  it('repo access failure surfaces an actionable auth toast and rethrows', async () => {
    ...
    await withSyncEnv({ github: gh }, async ({ events }) => {
      ...
      assert.ok(t.some((x) => x.level === 'error' && /sign in/i.test(x.message)), 'auth toast points at sign-in');
    });
  });
});
...
describe('sync: commitDocument (FR-GH.3)', () => {
  ...
  it('preserves edits typed while a commit is in flight', async () => {
    const gh = fakeGitHub({ branches: ['draft'] });
    const doc = await makeDoc('sent', { github: boundGithubField({ sha: null }) });
    let live = 'sent'; const put = gh.putFile;
    gh.putFile = async args => { const result = await put(args); live = 'newer edits'; return result; };
    await withSyncEnv({ github: gh, bridge: { getLocalContent: () => live } }, async ({ events }) => {
      const saved = await sync.commitDocument(doc.id);
      assert.equal(saved.content, 'newer edits');
      assert.equal(gh._state.files[BOUND.path].content, 'sent');
      assert.equal(lastStatus(events).state, 'local-changes');
    });
  });
  ...
  it('rate-limit failure surfaces a sign-in guidance', async () => {
    ...
    await withSyncEnv({ github: gh }, async ({ events }) => {
      ...
      assert.ok(toasts(events).some((t) => /rate limit/i.test(t.message) && /sign in/i.test(t.message)));
    });
  });
