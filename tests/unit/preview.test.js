...
describe('preview: createPreviewPane render() happy path', () => {
  it('writes renderPost html into the iframe srcdoc and injects styles once', async () => {
    ...
    try {
      ...
      assert.equal(iframe.getAttribute('sandbox'), 'allow-scripts allow-popups allow-forms allow-modals allow-downloads');
...
describe('preview: opaque origin geometry bridge', () => {
  it('reports geometry and scrolls without access to the editor document', async () => {
    const mount = makeMount();
    const pane = createPreviewPane({ mount, buildContext: async () => ({}), renderPost: async () => ({ html: '<html><body><h1>Heading</h1><div style="height:3000px"></div><script>try { parent.document.body.dataset.previewEscaped="yes"; } catch {}<\/script></body></html>', diagnostics: [] }) });
    const until = async predicate => {
      for (let i = 0; i < 100 && !predicate(); i++) await new Promise(resolve => setTimeout(resolve, 20));
      assert.ok(predicate(), 'preview bridge responded');
    };
    try {
      await pane.render({ content: '', path: 'test.md' });
      await until(() => pane.getMetrics()?.anchors.length > 0);
      assert.equal(pane.getFrame().contentDocument, null);
      assert.ok(!document.body.dataset.previewEscaped);
      pane.scrollTo(300);
      await until(() => pane.getMetrics()?.y >= 250);
    } finally { pane.destroy(); mount.remove(); delete document.body.dataset.previewEscaped; }
  });
});
