...
import { savePackage, pullPackage, readRemotePackage, savedBinding } from './package-sync.js';
import { sourceFields } from './package-validation.js';
...
function toastGitHubError(err) {
  ...
  if (kind === 'auth') {
    toast('GitHub access is unavailable. Sign in from the sync panel and check that this account can write to the repository.', 'error');
  } else if (kind === 'rate-limit') {
    toast('GitHub API rate limit reached. Sign in from the sync panel or wait before trying again.', 'warning');
...
export async function bindDocument(docId, { owner, repo, branch, path } = {}) {
  ...
  try {
    // Reconnecting the same destination must not reset its conflict baseline.
    if (record.github?.owner === owner && record.github?.repo === repo && record.github?.branch === branch && record.path === path) return record;
    ...
    if (record.manifest) {
      if (probe && probe !== 'not-modified') throw new Error('A file already occupies that destination. Choose another path for this workspace.');
      const fresh = await _deps.store.docs.get(docId);
      const saved = await _deps.store.workspaces.mutate(docId, { expectedVersion: fresh.workspaceVersion || 0, content: localContentOf(fresh), manifest: { ...fresh.manifest, entryPath: path }, github: {owner,repo,branch,sha:null,syncedAt:null} });
      emitStatus(saved); return saved;
    }
...
export async function commitDocument(docId, { message } = {}) {
  ...
  if (record.workspaceUnsupported) throw new Error('This workspace format cannot be saved safely. Export the Markdown or use a compatible editor.');
  ...
  const local = localContentOf(record);
  if (record.manifest) { try { const saved = await savePackage(docId, _deps.bridge); emitStatus(saved); return saved; } catch(error) { toast(error.message, 'error'); throw error; } }
  ...
  try {
    ...
    const saved = await finishSync(docId, {
      owner, repo, branch, sha: result.sha, content: local, preserveNewer: true,
    });
...
export async function pullDocument(docId) {
  const record = await _deps.store.docs.get(docId);
  if (record?.manifest) { const saved = await pullPackage(docId, _deps.bridge); emitStatus(saved); return saved; }
...
export function parseGitHubFileRef(ref, binding = null) {
  ...
  if (m) return { owner: m[1], repo: m[2], branch: decodeURIComponent(m[3]), path: decodeURIComponent(m[4]) };
  ...
  if (parts.length >= 4 && !s.startsWith('_') && !s.startsWith('assets/')) {
    // owner/repo/branch/path…
    return { owner: parts[0], repo: parts[1], branch: decodeURIComponent(parts[2]), path: parts.slice(3).join('/') };
  }
...
export async function openFromGitHub({ owner, repo, branch = 'main', path } = {}) {
  ...
  if (sourceFields(file.content).storykit_workspace) {
    try {
      const remote = await readRemotePackage({owner,repo,branch,path});
      const created = await _deps.store.workspaces.importDocument(remote);
      const saved = await _deps.store.docs.update(created.id, { github: savedBinding({owner,repo,branch}, remote, remote) });
      emitStatus(saved);return {docId:created.id,created:true};
    } catch(error) {
      const created=await _deps.store.docs.create({title:titleFromContent(file.content,basename(path)),path,content:file.content});
      const saved=await _deps.store.docs.update(created.id,{workspaceUnsupported:true,github:{owner,repo,branch,sha:file.sha}});
      toast(`Opened Markdown for recovery. Package unavailable: ${error.message}`, 'warning');
      emitStatus(saved);return {docId:created.id,created:true};
    }
  }
...
export async function checkRemote(docId) {
  ...
  if (record.manifest && gh.package) {
    try { const head = await _deps.github.getBranchHead(gh); const saved = await _deps.store.docs.update(docId, { github: {...gh, remoteChanged: head.sha !== gh.package.commit} }); emitStatus(saved); return head.sha === gh.package.commit ? 'unchanged' : 'changed'; } catch { return 'skipped'; }
  }
 ...
 */
async function finishSync(docId, { owner, repo, branch, sha, content, etag, preserveNewer = false }) {
  const github = { owner, repo, branch, sha, remoteChanged: false };
  if (etag) github.etag = etag;
  const fields = {};
  const latest = preserveNewer ? localContentOf(await _deps.store.docs.get(docId)) : content;
  const changed = preserveNewer && latest !== content;
  if (typeof content === 'string') fields.content = changed ? latest : content;
  let saved = await updateAnchored(docId, fields, github);
  if (changed) saved = await _deps.store.docs.update(docId, { github: { ...saved.github, syncedAt: new Date(Date.parse(saved.updatedAt) - 1).toISOString() } });
  emitStatus(saved);
  return saved;
}
