...
import { canonicalJSON, postDirectory, setSourceField } from './package-validation.js';
...
export function deriveSyncStatus(doc) {
  ...
  if (doc.manifest) return gh.package && gh.package.content === doc.content && canonicalJSON(gh.package.manifest) === canonicalJSON(doc.manifest) ? 'synced' : 'local-changes';
...
export function createDocList({ mount, store, bus, onOpen, onSync, onSyncSetup, onOpenRemote } = {}) {
  ...
  if (typeof onSyncSetup === 'function') {
    ...
    setupBtn.title = 'GitHub sign-in and sync settings';
  ...
  async function createNewPost(rawTitle) {
    ...
    const content = setSourceField(fillTemplate(fallbackTemplate, { title, date: toDateString(date) }), 'media_subpath', postDirectory(path));
    try {
  ...
  function exportDoc(docRecord) {
    try {
      ...
      notify('toast', { message: docRecord.manifest ? `Exported ${filename} without images. Use Story files → Export story package to include them.` : `Exported ${filename}`, level: 'success' });
    } catch (err) {
  ...
  function renderRenameForm(docRecord) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if(docRecord.manifest){
          if(docRecord.github)throw new Error('Duplicate this bound workspace to use a new destination.');
          const {validateManifest}=await import('./package-validation.js');
          const current=await store.docs.get(docRecord.id);
          const manifest=validateManifest({...current.manifest,entryPath:path},current.content);
          await store.workspaces.mutate(docRecord.id,{expectedVersion:current.workspaceVersion,content:current.content,manifest});
        }else await store.docs.update(docRecord.id, { path });
        renamingId = null;
