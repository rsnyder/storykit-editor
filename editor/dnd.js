...
function makeDomEventHandlers(onNotice, onFiles) {
  ...
  return EditorView.domEventHandlers({
    ...
    drop(event, view) {
      ...
      if (hasDroppedFiles(dataTransfer)) {
        event.preventDefault();
        if (onFiles) onFiles(Array.from(dataTransfer.files), view, event.type === 'drop' ? resolveDropPos(view, event) : view.state.selection.main.from);
        else notify(LOCAL_IMAGE_NOTICE);
        return true;
      }
 ...
 */
export function dndExtension({ onNotice, onFiles } = {}) {
  return [
    dndTheme,
    dragChipPlugin,
    hintField,
    ...
    pasteExpiryPlugin,
    dismissOnEscape,
    makeDomEventHandlers(onNotice, onFiles),
  ];
}
