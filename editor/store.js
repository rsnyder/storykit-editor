 ...
 * "storykit-editor", version 2, via `idb` (esm.sh pin, see
...
const FORCED_SNAPSHOT_REASONS = new Set(['pre-pull', 'pre-conflict', 'pre-restore', 'pre-files', 'manual']);
...
function openDatabase() {
  return openDB(DB_NAME, DB_VERSION, {
    blocked() { globalThis.dispatchEvent?.(new CustomEvent('storykit-storage-blocked')); },
    blocking(_old, _next, event) { event.target.close(); dbPromise = null; },
    upgrade(db) {
      if (!db.objectStoreNames.contains('assetBytes')) db.createObjectStore('assetBytes');
...
export const docs = {
   ...
   */
  async update(id, patch = {}, {expectedContent,expectedVersion} = {}) {
    const db = await getDB();
    const tx = db.transaction('documents', 'readwrite');
    const existing = await tx.store.get(id);
    if (!existing) throw new Error(`docs.update: no document with id "${id}"`);
    if((expectedVersion!==undefined&&(existing.workspaceVersion||0)!==expectedVersion)||(expectedContent!==undefined&&existing.content!==expectedContent&&existing.content!==patch.content)){
      tx.abort();await tx.done.catch(()=>{});throw new Error('This story changed in another tab. Your current text is still in the editor; export it before reopening the saved version.');
    }
    const keys = Object.keys(patch).filter((k) => k !== 'id');
    const metadataOnly = keys.length > 0 && keys.every((k) => k === 'github');
    const next = {
      ...existing, ...patch, id: existing.id,
      workspaceVersion: (existing.workspaceVersion || 0) + (metadataOnly ? 0 : 1),
  ...
  async duplicate(id) {
    ...
    if (existing.manifest) {
      const { copyPackage } = await import('./workspace-copy.js');
      const snapshot = await workspaces.capture(id);
      return workspaces.importDocument(copyPackage({content:snapshot.doc.content,manifest:snapshot.doc.manifest,blobs:snapshot.blobs}));
    }
    const now = new Date().toISOString();
    const copy = {
      ...existing,
...
export const revisions = {
  ...
  async snapshot(docId, content, reason) {
    ...
    const record = {
      ...(doc?.manifest ? { manifest: doc.manifest, workspaceId: doc.workspaceId, path: doc.path, workspaceVersion: doc.workspaceVersion } : { manifest: null }),
      id: ulid(),
      docId,
      content,
      reason: reason || 'auto',
    ...
    await tx.objectStore('revisions').put(record);
    await tx.done;
...
export function createAutosaver(docId, { debounceMs = 1500, onSave } = {}) {
  let baseline = docs.get(docId).then(doc=>doc?.content);
  ...
  async function writeNow() {
    ...
    latestContent = null;
    const saved = await docs.update(docId, { content }, {expectedContent:await baseline});
    baseline=Promise.resolve(saved.content);
  ...
  function push(content) {
    ...
    timer = setTimeout(() => {
      ...
      pending = writeNow().catch((err) => {
        globalThis.dispatchEvent?.(new CustomEvent('storykit-storage-error',{detail:err.message}));
...
/** All package state changes share one IDB authority. Hash/decode beforehand;
 * no non-IDB await is allowed inside the mutation transaction. */
const storedBlob = value => value instanceof Blob ? value : value ? new Blob([value.bytes],{type:value.mime}) : null;
const byteRecords = blobs => Promise.all(blobs.map(async b=>({contentHash:b.contentHash,bytes:new Uint8Array(await b.blob.arrayBuffer()),mime:b.blob.type})));
export const workspaces = {
  async bytes(hash) { return storedBlob(await (await getDB()).get('assetBytes', hash)); },
  async mutate(id, { expectedVersion, content, manifest, blobs = [], reason = 'pre-files', github }) {
    const prepared=await byteRecords(blobs);
    const db=await getDB(),tx=db.transaction(['documents','revisions','assetBytes'],'readwrite');
    const documents=tx.objectStore('documents'),current=await documents.get(id);
    if(!current || (current.workspaceVersion||0)!==expectedVersion) {
      tx.abort();await tx.done.catch(()=>{});throw new Error('This story changed in another operation or tab. Reload it before changing files.');
    }
    if(reason) await tx.objectStore('revisions').put({id:ulid(),docId:id,content:current.content,
      manifest:current.manifest||null,path:current.path,workspaceId:current.workspaceId||null,
      reason,createdAt:new Date().toISOString()});
    for(const {contentHash,...record} of prepared) await tx.objectStore('assetBytes').put(record,contentHash);
    for(const file of manifest?.files||[]) {
      if(!await tx.objectStore('assetBytes').get(file.contentHash)) {
        tx.abort();await tx.done.catch(()=>{});throw new Error(`Missing bytes for ${file.path}; locate the original file or import a package.`);
      }
    }
    const next={...current,content,manifest,workspaceId:manifest?.workspaceId||null,
      path:manifest?.entryPath||current.path,workspaceVersion:expectedVersion+1,updatedAt:new Date().toISOString(),
      ...(github!==undefined?{github}: {})};
    await documents.put(next);await tx.done;await revisions.prune(id);await workspaces.collect();return next;
  },
  async capture(id) {
    const db=await getDB(),tx=db.transaction(['documents','assetBytes'],'readonly');
    const doc=await tx.objectStore('documents').get(id);
    if(!doc)throw new Error('Story was closed or removed.');
    const blobs=[];
    for(const f of doc.manifest?.files||[]) {
      const blob=storedBlob(await tx.objectStore('assetBytes').get(f.contentHash));
      if(!blob)throw new Error(`Missing file: ${f.path}. Locate it before saving or exporting.`);
      blobs.push({...f,blob});
    }
    await tx.done;return {doc,blobs};
  },
  async importDocument({content,manifest,blobs}) {
    const prepared=await byteRecords(blobs);
    const db=await getDB(),tx=db.transaction(['documents','assetBytes'],'readwrite');
    const all=await tx.objectStore('documents').getAll();
    if(all.some(d=>d.workspaceId===manifest.workspaceId)) {tx.abort();await tx.done.catch(()=>{});throw new Error('This workspace is already in this browser. Open it from the document list.');}
    const now=new Date().toISOString(),doc={id:ulid(),content,manifest,path:manifest.entryPath,
      workspaceId:manifest.workspaceId,workspaceVersion:1,github:null,title:deriveTitle(content,manifest.entryPath),createdAt:now,updatedAt:now,revisions:[]};
    for(const {contentHash,...record} of prepared)await tx.objectStore('assetBytes').put(record,contentHash);
    for(const f of manifest.files)if(!await tx.objectStore('assetBytes').get(f.contentHash)){tx.abort();await tx.done.catch(()=>{});throw new Error('Incomplete package.');}
    await tx.objectStore('documents').put(doc);await tx.done;return doc;
  },
  async operation(record,blobs=[]) {
    const prepared=await byteRecords(blobs),db=await getDB(),tx=db.transaction(['operations','assetBytes'],'readwrite');
    for(const {contentHash,...value} of prepared)await tx.objectStore('assetBytes').put(value,contentHash);
    await tx.objectStore('operations').put(record);await tx.done;
  },
  async operations(id) { return (await (await getDB()).getAll('operations')).filter(o=>o.docId===id); },
  async collect() {
    const db=await getDB(),tx=db.transaction(['documents','revisions','operations','assetBytes'],'readwrite'),keep=new Set();
    for(const name of ['documents','revisions','operations']){
      let record=await tx.objectStore(name).openCursor();
      while(record){const r=record.value;if(name!=='operations'||r.state==='uncertain')for(const f of (r.manifest||r.snapshot?.manifest)?.files||[])keep.add(f.contentHash);record=await record.continue();}
    }
    let cursor=await tx.objectStore('assetBytes').openKeyCursor();let removed=0;
    while(cursor){if(!keep.has(cursor.key)){await tx.objectStore('assetBytes').delete(cursor.key);removed++;}cursor=await cursor.continue();}await tx.done;return removed;
  },
};
