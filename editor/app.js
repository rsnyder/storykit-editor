...
import { restorePackage } from './workspace.js';
...
function buildExtraExtensions() {
  return [
    ...
    dnd.dndExtension({ onNotice: (n) => emit('toast', n), onFiles: (files, view, pos) => filesPanel?.ingest(files, { insert: true, pos }).catch(e => showToast({message:e.message,level:'error'})) }),
...
function previewArgsFor(content) {
  return { content, path: previewPathFor(currentDocRecord), binding: appState.binding, docId: appState.currentDocId };
}
...
function rebuildScrollSync() {
  ...
  const metrics = previewHandle.getMetrics?.();
  ...
  for (const anchor of metrics.anchors) {
    if (anchor.viewer) { pvAnchors.push({ key: `v:${anchor.id}`, top: anchor.top }); continue; }
    const norm = scrollsync.normalizeHeadingText(anchor.text);
    if (!norm) continue;
    const n = (rebuildScrollSync._seen.get(norm) || 0) + 1;
    rebuildScrollSync._seen.set(norm, n);
    pvAnchors.push({ key: `h:${n}:${norm}`, top: anchor.top });
  }
  rebuildScrollSync._seen.clear();
  const totalLines = editorHandle.view.state.doc.lines;
  scrollSyncState.map = scrollsync.buildScrollMap(srcAnchors, pvAnchors, totalLines, metrics.height);
  ...
  const onPreviewScroll = () => {
    ...
    scrollSyncSchedule('preview', () => {
      ...
      const lf = scrollsync.previewToSource(scrollSyncState.map, previewHandle.getMetrics()?.y || 0);
  ...
  iframe.addEventListener('sk-preview-scroll', onPreviewScroll);
  scrollSyncState.detach = () => iframe.removeEventListener('sk-preview-scroll', onPreviewScroll);
}
...
function onEditorScrollForSync() {
  ...
  scrollSyncSchedule('editor', () => {
    ...
    if (Math.abs((previewHandle.getMetrics()?.y || 0) - target) > 2) previewHandle.scrollTo(target);
  });
}
...
function wireStatusBar() {
  ...
  bus.addEventListener('doc:changed', () => {
    ...
    if(currentDocRecord.manifest)statusBar.setSyncState(doclist.deriveSyncStatus({...currentDocRecord,content:editorHandle?.getContent()??currentDocRecord.content}));
    else if (statusBar.getState() === 'synced') statusBar.setSyncState('local-changes');
  });
}
...
function wireToasts() {
  ...
  const refreshBadge = async () => {
    ...
    try {
      ...
      if (rec) { currentDocRecord = rec; const display=rec.manifest?{...rec,content:editorHandle?.getContent()??rec.content}:rec;updateTopbarSyncBadge(display);if(rec.manifest)reflectSyncStatus(display); }
    } catch { /* transient */ }
  };
...
function buildSyncPanel() {
  ...
  const tokenStatus = h('p', { class: 'sk-field-note', id: 'sync-auth-status', role: 'status' });
  const signInBtn = h('button', { type: 'button', class: 'btn btn-primary btn-sm', id: 'sync-sign-in', text: 'Sign in with GitHub' });
  const signOutBtn = h('button', { type: 'button', class: 'btn btn-sm', id: 'sync-sign-out', text: 'Sign out' });
  signInBtn.addEventListener('click', async () => {
    signInBtn.disabled = true;
    try {
      if (currentAutosaver && editorHandle) currentAutosaver.push(editorHandle.getContent());
      await currentAutosaver?.flush();
      sessionStorage.setItem('storykit.sync-return', '1');
      await auth.signIn();
    } catch (error) { tokenStatus.textContent = error.message; signInBtn.disabled = false; }
  });
  signOutBtn.addEventListener('click', async () => {
    try { await auth.signOut(); loadedFor = ''; writableRepos = []; refresh(); }
    catch (error) { tokenStatus.textContent = error.message; }
  });
  const tokenSection = h('section', { class: 'sk-dialog-section' }, [
    h('h3', { class: 'sk-dialog-h', text: 'Step 1 · GitHub account' }), tokenStatus,
    h('div', { class: 'sk-field-row' }, [signInBtn, signOutBtn]),
    h('p', { class: 'sk-field-note', text: 'Sign-in is optional. GitHub will request access to public and private repositories. Nothing is uploaded by signing in. Signing out keeps your local drafts.' }),
  ]);
  let writableRepos = [], loadedFor = '', hydratedFor = '', loadingVersion = 0;
  const repoSelect = h('select', { class: 'sk-input', id: 'sync-repositories', size: 6, 'aria-label': 'Writable repository' });
  const repoSearch = h('input', { type: 'search', class: 'sk-input', id: 'sync-repo-search', placeholder: 'Find a repository', 'aria-label': 'Find a repository' });
  const repoStatus = h('p', { class: 'sk-field-note', id: 'sync-repo-status', role: 'status' });
  const refreshRepos = h('button', { type: 'button', class: 'btn btn-sm', text: 'Refresh repositories' });
  refreshRepos.addEventListener('click', () => { loadedFor = ''; refresh(); });
  function renderRepos() {
    const wanted = `${ownerInput.value}/${repoInput.value}`.toLowerCase();
    const items = writableRepos.filter(r => r.fullName.toLowerCase().includes(repoSearch.value.toLowerCase()));
    repoSelect.replaceChildren(new Option('Choose a repository', ''), ...items.map(r => new Option(r.fullName + (r.private ? ' · private' : ' · public'), String(r.id))));
    const selected = items.find(r => r.fullName.toLowerCase() === wanted);
    repoSelect.value = selected ? String(selected.id) : '';
  }
  repoSearch.addEventListener('input', renderRepos);
  const branchOptions = h('datalist', { id: 'sync-branches' });
  async function loadBranches(repo, preserve = false) {
    const version = loadingVersion;
    try {
      const branches = await github.listBranches({ owner: repo.owner, repo: repo.name });
      if (version !== loadingVersion || ownerInput.value !== repo.owner || repoInput.value !== repo.name) return;
      branchOptions.replaceChildren(...branches.map(b => new Option(b.name, b.name)));
      if (!preserve) branchInput.value = repo.defaultBranch;
      if (preserve && !branches.some(b => b.name === branchInput.value)) repoStatus.textContent = 'The original branch is missing. Choose a branch explicitly before connecting.';
    } catch (error) { repoStatus.textContent = error.message; }
  }
  repoSelect.addEventListener('change', () => {
    const repo = writableRepos.find(r => String(r.id) === repoSelect.value);
    if (!repo) return;
    ownerInput.value = repo.owner; repoInput.value = repo.name;
    branchInput.value = ''; loadBranches(repo);
  });
  async function loadRepositories() {
    const version = ++loadingVersion;
    repoStatus.textContent = 'Loading writable repositories…';
    try {
      const items = await github.listRepositories();
      if (version !== loadingVersion) return;
      writableRepos = items; renderRepos();
      const match = items.find(r => r.fullName.toLowerCase() === `${ownerInput.value}/${repoInput.value}`.toLowerCase());
      repoStatus.textContent = currentDocRecord?.github && !match
        ? 'This account cannot write to the original repository. Choose another destination; your original binding is retained.'
        : `${items.length} writable repositories`;
      if (match) await loadBranches(match, true);
    } catch (error) { if (version === loadingVersion) repoStatus.textContent = error.message; }
  }
  ...
  const branchInput = /** @type {HTMLInputElement} */ (h('input', { type: 'text', class: 'sk-input', id: 'sync-branch', list: 'sync-branches', placeholder: 'branch (created if new)' }));
  ...
  bindBtn.addEventListener('click', async () => {
    ...
    try {
      const choice = writableRepos.find(r => r.owner === binding.owner && r.name === binding.repo);
      if (!choice) throw new Error('Choose a writable repository from the list.');
      const branches = await github.listBranches(binding);
      if (!branches.some(b => b.name === binding.branch) && !window.confirm(`Create branch “${binding.branch}” in ${binding.owner}/${binding.repo}?`)) return;
  ...
  const bindingSection = h('section', { class: 'sk-dialog-section' }, [
    ...
    bindingNote, repoSearch, repoSelect, repoStatus, refreshRepos, branchOptions,
    h('div', { class: 'sk-field-grid' }, [
      h('span', { hidden: true }, [ownerInput, repoInput]),
  ...
  commitBtn.addEventListener('click', async () => {
    const docId = appState.currentDocId;
    if (!docId) return;
    commitBtn.disabled = true; signOutBtn.disabled = true; bindBtn.disabled = true;
    try {
      ...
      if (saved) {
        ...
        showResult(`✓ Committed to ${gh.owner}/${gh.repo} on ${gh.branch} at ${new Date().toLocaleTimeString()}. ${doclist.deriveSyncStatus(saved) === 'local-changes' ? 'You have newer local edits to save.' : 'Your document and GitHub now match — you can close this dialog.'}`);
      }
    } catch(error) { showResult(error.message || 'The operation could not be completed. Your local story is retained.'); }
    finally { signOutBtn.disabled = false; refreshSyncPanel(); }
  });
  ...
  dialog.append(body);
  dialog.addEventListener('close', () => { resultNote.hidden = true; });
  ...
  function refresh() {
    const session = auth.getSession(), signedIn = !!session?.user;
    tokenStatus.textContent = signedIn ? `Signed in as @${session.user.login}` : session?.configured === false ? 'GitHub login is not configured on this host. Local editing and export remain available.' : 'Sign in when you are ready to save to GitHub.';
    signInBtn.hidden = signedIn; signInBtn.disabled = session?.configured === false;
    signOutBtn.hidden = !signedIn;
    repoSearch.disabled = repoSelect.disabled = refreshRepos.disabled = !signedIn;
    if (!signedIn) { loadingVersion++; writableRepos = []; loadedFor = ''; renderRepos(); branchOptions.replaceChildren(); }
    // Binding fields prefilled from the open doc.
    const rec = currentDocRecord;
    const gh = rec && rec.github;
    const destinationKey = JSON.stringify([appState.currentDocId, gh?.owner, gh?.repo, gh?.branch, rec?.path]);
    if (destinationKey !== hydratedFor) {
      hydratedFor = destinationKey;
      ownerInput.value = gh?.owner || '';
      repoInput.value = gh?.repo || '';
      branchInput.value = gh?.branch || '';
      pathInput.value = rec?.path || '';
    }
    bindBtn.textContent = gh ? 'Update binding' : 'Connect';

    // The panel is reachable with NO document open (the doc-list "GitHub"
    // button — the login entry point on first run), so say what this
    ...
    bindingNote.textContent = noDoc
      ...
      : bound
        ...
        : 'Not connected yet. Choose a repository below and click Connect to enable committing and pulling changes for this document.';
    syncSection.hidden = !bound;
    commitBtn.disabled = !bound || !signedIn;
    ...
    bindBtn.disabled = noDoc || !signedIn;
    const key = `${session?.user?.id}:${appState.currentDocId}`;
    if (signedIn && key !== loadedFor) { loadedFor = key; loadRepositories(); }
  }
  ...
  const rows = revs.map((rev) => {
    ...
    restoreBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!Object.hasOwn(rev,'manifest') && currentDocRecord?.manifest && !window.confirm('This legacy revision has no file snapshot. Restore its text and retain current files for recovery?')) return;
        const saved = await restorePackage(docId, rev, current);
        if (editorHandle) replaceOpenBuffer(docId, saved.content);
...
function wireSyncPanel() {
  ...
  sync.setEditorBridge({
    ...
    getWorkspaceVersion: docId => docId===appState.currentDocId ? currentDocRecord?.workspaceVersion||0 : undefined,
...
export async function init() {
  ...
  wireSyncPanel();
  wireCommandSurface();
  window.addEventListener('storykit-storage-error',event=>emit('toast',{message:event.detail,level:'error'}));
  filesPanel = createFilesPanel({getDocId:()=>appState.currentDocId,getVersion:()=>currentDocRecord?.workspaceVersion||0,getView:()=>editorHandle?.view,
    update:(id,content)=>{replaceOpenBuffer(id,content);bus.dispatchEvent(new CustomEvent('doc:saved',{detail:{docId:id}}));refreshPreviewForCurrentMode();},
    notify:(message,level)=>showToast({message,level}),openDoc});
  const fileButton=document.createElement('button');fileButton.type='button';fileButton.className='btn btn-sm';fileButton.textContent='Story files';fileButton.id='story-files-btn';fileButton.onclick=()=>filesPanel.open();
  document.getElementById('toolbar-mount')?.append(fileButton);
  const upgradeNotice=document.createElement('p');upgradeNotice.hidden=true;upgradeNotice.setAttribute('role','alert');upgradeNotice.id='storage-upgrade-notice';upgradeNotice.textContent='Close or reload older editor tabs so browser storage can be upgraded. Your drafts are retained.';document.getElementById('sidebar')?.append(upgradeNotice);
  window.addEventListener('storykit-storage-blocked',()=>{upgradeNotice.hidden=false;});
  ...
  if (dlMount) {
    try {
      docListHandle = doclist.createDocList({
        mount: dlMount,
        store: { docs: store.docs, workspaces: store.workspaces },
        bus,
  ...
  try {
    ...
    const arrivingWithIntent = new URLSearchParams(window.location.search).has('open') || !!sessionStorage.getItem('storykit.pending-open');
  ...
  try {
    ...
    if (openParam) {
      ...
      if (repoParam && !/^https?:/i.test(openParam)) {
        ...
        ref = `${owner}/${repo}/${encodeURIComponent(params.get('branch') || 'main')}/${openParam.replace(/^\/+/, '')}`;
      }
  ...
  try {
    const pending = sessionStorage.getItem('storykit.pending-open');
    if (pending && auth.getSession()?.user) {
      if (await openRemoteRef(pending)) sessionStorage.removeItem('storykit.pending-open');
    }
    if (sessionStorage.getItem('storykit.sync-return')) {
      sessionStorage.removeItem('storykit.sync-return');
      openSyncPanel();
    }
    const params = new URLSearchParams(location.search);
    if (params.has('auth')) {
      const outcome = params.get('auth');
      if (outcome !== 'success') showToast({ message: outcome === 'cancelled' ? 'Sign-in canceled. Your local drafts are safe.' : 'GitHub sign-in did not finish. Please try again.', level: 'warning' });
      params.delete('auth'); params.delete('reason');
      history.replaceState(null, '', location.pathname + (params.size ? '?' + params : ''));
    }
  } catch { /* Session storage may be unavailable; keep the local editor open. */ }
...
export async function openRemoteRef(ref) {
  let parsed;
  try { parsed = await github.resolveFileURL(ref) || sync.parseGitHubFileRef(ref, appState.binding); }
  catch (error) {
    showToast({ message: error.message, level: 'warning' });
    if (!auth.getSession()?.user) {
      try { sessionStorage.setItem('storykit.pending-open', ref); } catch {}
      openSyncPanel();
    }
    return false;
  }
  ...
  try {
    ...
    if (!auth.getSession()?.user) {
      try { sessionStorage.setItem('storykit.pending-open', ref); } catch { /* URL remains available */ }
      openSyncPanel();
    }
