 ...
 * The authenticated editor uses an opaque-origin preview. A validated geometry
 * bridge reports scroll positions and anchors without exposing the parent DOM;
 * scroll positions are restored through messages after a re-render.
 *
 * ─────────────────────────────────────────────────────────────────────────
...
import { workspaces, docs } from './store.js';
import { localPreviewSource } from './file-references.js';
import { attachPreviewResources, previewResourceScript } from './preview-resources.js';
...
// Opaque origin prevents rendered document scripts from accessing auth or drafts.
const SANDBOX_ATTRS = 'allow-scripts allow-popups allow-forms allow-modals allow-downloads';
...
export function createPreviewPane({
  ...
  // The opaque preview reports geometry only. No draft, auth, or storage APIs
  // are exposed across this boundary; all messages are tied to this frame.
  let metrics = null, bridgeId = 0, pendingScroll = null;
  function scrollToPreview(top) {
    if (Number.isFinite(top)) iframe.contentWindow?.postMessage({ kind: 'sk-preview-scroll', id: bridgeId, top: Math.max(0, top) }, '*');
  }
  function receiveMetrics(event) {
    const data = event.data;
    if (destroyed || event.source !== iframe.contentWindow || data?.kind !== 'sk-preview-metrics' || data.id !== bridgeId) return;
    if (!Number.isFinite(data.y) || !Number.isFinite(data.height) || !Array.isArray(data.anchors) || data.anchors.length > 5000) return;
    if (!data.anchors.every(a => Number.isFinite(a.top) && typeof a.text === 'string' && a.text.length <= 1000 && typeof a.id === 'string' && a.id.length <= 200 && typeof a.viewer === 'boolean')) return;
    const first = !metrics;
    metrics = { y: Math.max(0, data.y), height: Math.max(0, data.height), anchors: data.anchors };
    if (first && pendingScroll !== null) { scrollToPreview(pendingScroll); pendingScroll = null; }
    if (typeof onRendered === 'function') onRendered();
    iframe.dispatchEvent(new Event('sk-preview-scroll'));
  }
  window.addEventListener('message', receiveMetrics);
  function bridgeScript(id) {
    return `<script>(() => {
      const id = ${id}; let scheduled = false;
      const report = () => {
        scheduled = false;
        const root = document.querySelector('.post-content') || document.body;
        if (!root) return;
        const anchors = [...root.querySelectorAll('h1,h2,h3,h4,h5,h6,iframe[id]')].slice(0,5000).map(el => ({
          text: (el.textContent || '').slice(0,1000), id: (el.id || '').slice(0,200), viewer: el.tagName === 'IFRAME', top: el.getBoundingClientRect().top + scrollY
        }));
        parent.postMessage({ kind: 'sk-preview-metrics', id, anchors, y: scrollY, height: Math.max(0, document.documentElement.scrollHeight - innerHeight) }, '*');
      };
      const schedule = () => { if (!scheduled) { scheduled = true; requestAnimationFrame(report); } };
      addEventListener('message', e => { if (e.source === parent && e.data?.kind === 'sk-preview-scroll' && e.data.id === id && Number.isFinite(e.data.top)) scrollTo(0, e.data.top); });
      addEventListener('scroll', schedule, { passive: true }); addEventListener('resize', schedule); addEventListener('load', schedule);
      new ResizeObserver(schedule).observe(document.documentElement); schedule();
    })();<\/script>`;
  }

  // ── Writing the iframe + diagnostics panel ──────────────────────────────
  let releaseResources = null;
  function writeFrame(html, diagnostics, resources = null) {
    releaseResources?.(); releaseResources = null;
    if (destroyed) return;
    everRendered = true;
    overlay.hidden = true;
    pendingScroll = metrics?.y ?? null;
    metrics = null; bridgeId++;
    ...
    // Theme scripts expect Web Storage. In an opaque frame give them only
    // disposable, frame-local storage; never expose the editor's draft store.
    const storageShim = `<script data-storykit-preview-storage>(() => {
      for (const name of ['localStorage', 'sessionStorage']) {
        try { window[name].getItem(''); } catch {
          const data = new Map();
    ...
    if (resources) {
      ...
      const ids = new Set(resources.assigned.map(a => a.resourceId));
      for (const img of parsed.querySelectorAll('img')) {
        const id = img.getAttribute('src')?.replace(/^sk-local:/, '');
        if (ids.has(id)) { img.removeAttribute('src'); img.dataset.skResource = id; }
      }
      for (const link of parsed.querySelectorAll('a[href^="sk-local:"]')) {
        const id = link.getAttribute('href').slice(9);
        if (ids.has(id)) {link.dataset.skResource = id; link.setAttribute('download', resources.assigned.find(a => a.resourceId === id).displayName);}
      }
      let viewerURL = null;
      for (const viewer of parsed.querySelectorAll('iframe.embed-image')) {
        const url = new URL(viewer.src);
        const id = url.searchParams.get('src')?.replace(/^sk-local:/, '');
        if (ids.has(id)) {
          // Local resources use the viewer shipped with this editor, so their
          // protocol cannot lag behind an independently deployed public site.
          const localViewer = new URL('../assets/components/image.html', import.meta.url);
          url.protocol = localViewer.protocol; url.host = localViewer.host; url.pathname = localViewer.pathname;
          url.searchParams.delete('src'); url.searchParams.delete('manifest');
          url.searchParams.set('sk_resource', id); url.searchParams.set('sk_generation', resources.generation);
          viewer.src = url.href; viewer.dataset.skResource = id; viewerURL = url.origin + url.pathname;
        }
      }
      const mapURL = new URL('../assets/components/map.html', import.meta.url);
      for (const viewer of parsed.querySelectorAll('iframe.embed-map')) {
        const url = new URL(viewer.src);
        const localIds = (url.searchParams.get('geojson') || '').split('|').map(v => v.split('~')[0].trim())
          .filter(v => v.startsWith('sk-local:')).map(v => v.slice(9)).filter(id => ids.has(id));
        if (!localIds.length) continue;
        url.protocol = mapURL.protocol; url.host = mapURL.host; url.pathname = mapURL.pathname;
        url.searchParams.set('sk_resources', JSON.stringify([...new Set(localIds)]));
        url.searchParams.set('sk_resource', localIds[0]);
        url.searchParams.set('sk_generation', resources.generation);
        viewer.src = url.href; viewer.dataset.skResource = localIds[0];
        viewerURL ||= new URL('../assets/components/image.html', import.meta.url).href;
      }
      if (viewerURL) {
        // Keep host and consumer protocol versions aligned while preserving
        // the publication layout's initializer and its selected options.
        const hostURL = new URL('../assets/js/storykit.js', import.meta.url).href;
        let hasHost = false;
        const hostModule = /(?:https?:\/\/[^"'\s<>]+)?\/assets\/js\/storykit\.js(?:\?[^"'\s<>]*)?/g;
        for (const script of parsed.querySelectorAll('script')) {
          if (script.src && /\/assets\/js\/storykit\.js(?:\?|$)/.test(script.src)) {
            script.src = hostURL; hasHost = true;
          } else if (!script.src) {
            script.textContent = script.textContent.replace(hostModule, () => {hasHost = true; return hostURL;});
          }
        }
        if (!hasHost) {
          const host = parsed.createElement('script');
          host.type = 'module';host.src = hostURL;
          parsed.body.append(host);
        }
        for (const href of [
          'https://cdn.jsdelivr.net/npm/@shoelace-style/shoelace@2.18.0/cdn/themes/light.css',
        ]) {
          if (![...parsed.querySelectorAll('link[rel="stylesheet"]')].some(link => link.href === href)) {
            const link = parsed.createElement('link');link.rel = 'stylesheet';link.href = href;parsed.head.append(link);
          }
        }
      }
      html = '<!doctype html>' + parsed.documentElement.outerHTML;
      const script = previewResourceScript({ generation: resources.generation, assignments: resources.assigned, mapURL: mapURL.href, viewerURL: viewerURL || 'https://rsnyder.github.io/storykit-starter/assets/components/image.html' });
      html = html.replace(/<head\b[^>]*>/i, match => match + script);
      releaseResources = attachPreviewResources({ frame: iframe, generation: resources.generation, resources: resources.assigned });
    }
    const isolatedHtml = html.replace(/<head\b[^>]*>/i, match => match + storageShim);
    iframe.srcdoc = isolatedHtml + '<!--storykit-preview-bridge-->' + bridgeScript(bridgeId);
  ...
  async function render({ content, path, binding, docId } = {}) {
    ...
    try {
      ...
      let resources = null;
      if (docId) {
        const record = await docs.get(docId);
        if (record?.manifest) {
          const snapshot = await workspaces.capture(docId);
          resources = localPreviewSource(content, snapshot.doc.manifest, crypto.randomUUID());
          for (const resource of resources.assigned) resource.blob = snapshot.blobs.find(b => b.contentHash === resource.contentHash).blob;
      ...
      writeFrame(result.html, diagnostics.concat(result.diagnostics || []), resources);
    } catch (err) {
      if (destroyed || !tokenTracker.isCurrent(token)) return;
  ...
  return {
    ...
    schedule: scheduled,
    showEmpty,
    /** Opaque preview frame; geometry is available through getMetrics(). */
    getFrame() { return iframe; },
    getMetrics() { return metrics; },
    scrollTo: scrollToPreview,
    destroy() {
      releaseResources?.();
      if (destroyed) return;
      destroyed = true;
      scheduled.cancel();
      window.removeEventListener('message', receiveMetrics);
