...
function rewriteRelativeUrlsInString(html, rawBase, fileDir, deployedOrigin, baseurl, mediaSubpath) {
  ...
  function resolve(url, forAsset, isImage = false) {
    ...
    if (/^(https?:|data:|sk-local:|mailto:|tel:|#|\/\/)/i.test(u)) return u; // already absolute
...
export async function renderPost({ content, path, context }) {
  ...
  // Repositories can be authored before their first site deployment. Only
  // compiled theme resources use this fallback; source/media keep their bases.
  if (context.themeAssetOrigin) {
    const siteBase = (deployedOrigin + baseurl).replace(/\/$/, '');
    const themeBase = String(context.themeAssetOrigin).replace(/\/$/, '');
    if (siteBase && themeBase && siteBase !== themeBase) {
      const esc = siteBase.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      html = html.replace(
        new RegExp(`${esc}(/assets/(?:css/jekyll-theme-chirpy\\.css|js/dist/[a-z-]+\\.min\\.js))(?=[?"'\\s])`, 'g'),
        `${themeBase}$1`);
    }
  }
