#!/usr/bin/env python3
"""Render regression for the standalone StoryKit editor.

Prepare the framework fixture and build Jekyll before running. GitHub and
render-pipeline CDN requests are served from local fixtures; editor shell
ES modules are loaded live. --check compares editor-golden/; --capture
updates that baseline for deliberate rendering changes. Historical golden/
files remain the immutable pre-extraction preview-tool artifacts.
"""
...
GOLDEN_DIR = RENDER_DIR / "editor-golden"
...
def _poll_srcdoc(page, frame_el) -> str:
    ...
    while _time.monotonic() < deadline:
        ...
        if srcdoc and len(srcdoc) > 200 and "<html" in srcdoc[:400].lower():
            return re.sub(r"<script data-storykit-preview-storage>.*?</script>", "", srcdoc.split("<!--storykit-preview-bridge-->", 1)[0], flags=re.S)
        last = srcdoc
...
def run(args) -> int:
    ...
    live_requests = 0
    esm_live_total = 0
    capture_fn = capture_entry_editor
    entry_page = "editor/index.html"
...
def main(argv=None):
    p = argparse.ArgumentParser(description="Render-regression harness for the StoryKit editor")
    g = p.add_mutually_exclusive_group()
    g.add_argument("--capture", action="store_true",
                   help="write goldens to tests/render/editor-golden/")
    ...
    p.add_argument("--target", choices=["editor"], default="editor",
                   help="drive the standalone editor UI")
