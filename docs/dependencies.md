...
| fflate | 0.8.3 | Bounded ZIP story packages |
...
### Local story ZIP packages

`fflate` is pinned to `0.8.3` (`https://esm.sh/fflate@0.8.3`) for bounded streaming ZIP import and lossless ZIP export.

Server raster validation uses exact npm pins in `prototypes/github-auth/package-lock.json`: `jpeg-js@0.4.4`, `fast-png@8.0.0`, and `@jsquash/webp@1.5.0` (bundled WebAssembly decoder).
