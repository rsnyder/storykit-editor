...
* **FR-DND.6** **Unrecognized drops** of URLs MUST fall back to inserting a plain Markdown link, and supported local image files enter the browser-local ingestion flow defined in [local story files](editor-local-files-spec.md), without uploading. Drops must never be silently discarded.
...
* **FR-GH.7** Superseded by [local story files](editor-local-files-spec.md): managed images and Markdown use one expected-head package commit. Existing Markdown-only saves remain supported.
